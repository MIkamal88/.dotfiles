...
local bluetooth_widget = require("ui.bar.widgets.bluetooth")
local pacman_widget = require("ui.bar.widgets.pacman")
...
local widgetscollection = wibox.widget({
	{
		...
		barcontainer(bluetooth_widget),
		barcontainer(pacman_widget),
...
local function get_bar(s)
	...
	s.mywibar:setup({
		...
		shape = function(cr, w, h)
			gears.shape.rectangle(cr, w, h, 0)
		end,
		widget = wibox.container.background,
	})
end
