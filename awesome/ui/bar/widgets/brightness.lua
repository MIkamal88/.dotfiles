...
local function update_brightness_widget()
	awful.spawn.easy_async("sh -c 'brightnessctl -m | cut -d, -f4 | tr -d %'", function(stdout)
		local brightness_level = math.floor(stdout + 0.5)
		brightness_slider.value = tonumber(brightness_level)

		if brightness_level then
			brightness.text = tostring(brightness_level) .. "%"
		end
	end)
end

brightness:connect_signal("button::press", function(_, _, _, button)
	if button == 4 then
