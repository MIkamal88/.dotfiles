...
local function update_widget(stdout)
    ...
    for s in stdout:gmatch("[^\r\n]+") do
        ...
        if status ~= nil then
            ...
            table.insert(battery_info, batt)
            -- Only update time_remaining for the first battery
            if #battery_info == 1 and time and not time:match("remaining time: unknown") then
                time_remaining = time
            end
        else
    ...
    -- Only consider the first battery
    if #battery_info > 0 then
        local batt = battery_info[1]
        local charge = batt.charge
        local status = batt.status
